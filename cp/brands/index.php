...
// Handle brand deletion if requested
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    try {
        ...
        if ($productCount > 0) {
            $error = "Impossible de supprimer cette marque car elle est utilisée par {$productCount} produit(s)";
        } else {
...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-semibold text-gray-900">Marques</h1>
    <a href="<?= SITE_URL ?>/cp/brands/add.php" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
        Ajouter une marque
    </a>
</div>
            ...
            <?php if (isset($brands) && !empty($brands)): ?>
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="<?= SITE_URL ?>/cp/brands/edit.php?id=<?= $brand['id'] ?>" class="text-indigo-600 hover:text-indigo-900 mr-3">
                            Modifier
                        </a>
                        <a href="<?= SITE_URL ?>/cp/brands/index.php?delete=<?= $brand['id'] ?>" 
